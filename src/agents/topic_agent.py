...
from src.models import TopicResponseSchema
...
def get_topic_agent(model,prompt_query):
    topic_agent = create_react_agent(
        ...
        prompt=prompt_query,
        response_format = (
            "'topic' must describe the precise subtopic of the post in the most detailed and technical way possible. "
            "'position' must be one of: Initial, Middle, Final, StandAlone.",
            TopicResponseSchema,
        )
    )
    return topic_agent
